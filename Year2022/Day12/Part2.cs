...
namespace AdventOfCode.Year2022.Day12
{
  public class Part2 : IPuzzle
  {
    ...
    {
      int startX = input.Replace(Environment.NewLine, "").IndexOf('E') % input.IndexOf(Environment.NewLine);
      int startY = input.Replace(Environment.NewLine, "").IndexOf('E') / input.IndexOf(Environment.NewLine);
      ...
      for (int distance = 1; distance < input.Length; distance++)
      {
        ...
        foreach (var activeCoord in activeCoordinates)
        {
          ...
          if (x > 0 && explored[x - 1, y] == 0 && (map[y][x] - map[y][x - 1] == 1 || map[y][x - 1] >= map[y][x]))
            newActiveCoords.Add(new Tuple<int, int>(x - 1, y));

          if (x + 1 < map[0].Length && explored[x + 1, y] == 0 && (map[y][x] - map[y][x + 1] == 1 || map[y][x + 1] >= map[y][x]))
            newActiveCoords.Add(new Tuple<int, int>(x + 1, y));

          if (y > 0 && explored[x, y - 1] == 0 && (map[y][x] - map[y - 1][x] == 1 || map[y - 1][x] >= map[y][x]))
            newActiveCoords.Add(new Tuple<int, int>(x, y - 1));

          if (y + 1 < map.Length && explored[x, y + 1] == 0 && (map[y][x] - map[y + 1][x] == 1 || map[y + 1][x] >= map[y][x]))
            newActiveCoords.Add(new Tuple<int, int>(x, y + 1));
        }
